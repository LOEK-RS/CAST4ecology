...
reference_points <- st_read("data/reference_samples.gpkg", quiet = TRUE)
...
gd_rcv <- plot_geodist(reference_points,
                  modeldomain,
...
```{r ffs-selected}
plot_df = data.frame(labels = fct_rev(fct_inorder(c(paste(rfmodel_ffs$selectedvars[1:rfmodel_ffs$minVar], collapse = "\n + "),
                                    paste("+", rfmodel_ffs$selectedvars[-1:-rfmodel_ffs$minVar], sep = " ")))),
                         perf = rfmodel_ffs$selectedvars_perf,
                         perfse = rfmodel_ffs$selectedvars_perf_SE)


ggplot(plot_df, aes(x = perf, y = labels))+
  geom_point()+
  geom_segment(aes(x = perf - perfse, xend = perf + perfse,
